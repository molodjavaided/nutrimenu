...
> Адаптировано из MyResult. Держит фокус в фазе «полировка → первые платящие» — не распыляться.
...
6. **Перед новым LLM-промптом / сменой model-id** — один дешёвый пробный вызов на РЕАЛЬНЫХ данных
   (реальное блюдо для импорта, `scripts/probe_model.mjs`), не на предположении.
7. **Перед деплоем** — `node scripts/check_env.mjs` (ловит кириллицу в токенах, задвоенные слэши в URL).
8. **В конце сессии — `/wrap-session`** (дейли + HOME.md + auto-memory + graphify).
