...
export default function SettingsClient({ initialVenue }: { initialVenue: InitialVenue }) {
  const router = useRouter()
  const [saved, setSaved] = useState(false)
  const [deleteConfirm, setDeleteConfirm] = useState(false)
  const [deleting, setDeleting] = useState(false)
  const [allowAdminEdit, setAllowAdminEdit] = useState(initialVenue.allowAdminEdit ?? false)
  const [savingAdminEdit, setSavingAdminEdit] = useState(false)
  const slug = initialVenue.slug ?? null
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: {
      ...
      workingHours: initialVenue.workingHours ?? '',
    },
  })
