/**
 * Claude-based TTK validation, extraction, and PDF parsing logic.
 *
 * Modes:
 *   validateTTKDishes  — cross-checks/corrects parsed XLSX/CSV dishes against raw rows
 *   parsePDFTTK        — extracts dishes from a PDF or photo via Claude Vision
 *
...
function parseClaudeResponse(text: string): ValidationResult {
...
export async function parsePDFTTK(
  ...
  const IMAGE_TYPES = new Set<string>(['image/jpeg', 'image/jpg', 'image/png', 'image/webp', 'image/gif'])
  ...
  if (mimeType === 'application/pdf') {
    contentBlock = [
      ...
      { type: 'text', text: 'Извлеки все блюда и ингредиенты из этой ТТК.' },
    ]
    ...
    contentBlock = [
      {
        type: 'image',
        source: { type: 'base64', media_type: normalized as ImageMediaType, data: fileBase64 },
      },
      { type: 'text', text: 'Извлеки все блюда и ингредиенты из этой ТТК.' },
    ]
  } else {
